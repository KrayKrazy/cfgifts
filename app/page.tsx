...
export default function Home() {
  ...
  const buyOnWhatsApp = () => { setIsCheckingOut(true); return;
    const phone = "5511999999999"; // Replace with actual phone
    let text = '';

    if (activeProduct) {
      text = `OlÃ¡! Gostaria de encomendar a coleÃ§Ã£o pronta: *${activeProduct.name}*\nValor: ${fmt(activeProduct.price)}\n\nPoderia me informar sobre a entrega?`;
    ...
    <div className="min-h-screen text-slate-800 pb-24">
      ...
      <section className="bg-rose-50 border-b border-rose-100 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 py-20 flex flex-col items-center text-center relative z-10 animate-slideUp">
          ...
          <h2 className="font-serif text-4xl md:text-6xl font-bold text-slate-900 mb-6 max-w-3xl leading-tight">
            Surpreenda com um <i className="text-rose-600">presente Ãºnico.</i>
          </h2>
          <p className="text-slate-600 max-w-xl md:text-lg mb-10">
            Escolha uma de nossas coleÃ§Ãµes exclusivas prontas para emocionar, ou monte uma cesta do zero escolhendo cada detalhe.
          </p>
      ...
      <main id="catalogo" className="max-w-7xl mx-auto px-6 pt-20">
        <div className="flex flex-col items-center mb-16 text-center animate-fadeIn">
          <h2 className="font-serif text-3xl md:text-4xl font-bold text-slate-900">ColeÃ§Ãµes Afetivas</h2>
      ...
      {activeProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-6 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-5xl md:rounded-3xl shadow-2xl h-full md:h-[85vh] flex flex-col md:flex-row overflow-hidden relative animate-scaleUp">
            <button onClick={closeModal} className="absolute top-4 right-4 z-20 w-10 h-10 bg-white/80 md:bg-slate-100 rounded-full flex items-center justify-center hover:bg-rose-600 hover:text-white transition-colors">âœ•</button>
            ...
            <div className="w-full md:w-1/2 flex flex-col h-full overflow-y-auto bg-white p-6 md:p-12">
              ...
              <div className="bg-rose-50/50 border border-rose-100 rounded-2xl p-6 mb-8">
                <h4 className="font-bold text-sm uppercase tracking-wider text-slate-800 mb-4 flex items-center gap-2"><span>ðŸŽ</span> O kit contÃ©m:</h4>
                <ul className="space-y-3">
                  {activeProduct.items.map((item, i) => (
                    <li key={i} className="text-sm text-slate-700 flex items-start gap-2">
                      <span className="text-rose-400 mt-0.5">â€¢</span>
                      <span dangerouslySetInnerHTML={{ __html: item.replace(/O BoticÃ¡rio/g, '<b>O BoticÃ¡rio</b>').replace(/Eudora/g, '<b>Eudora</b>') }} />
                    </li>
                  ))}
                </ul>
              </div>
              <div className="mt-auto pt-6 border-t border-slate-100">
                <button onClick={buyOnWhatsApp} className="w-full bg-rose-600 text-white uppercase tracking-widest font-bold text-sm py-4 rounded-xl hover:bg-rose-700 transition-all">Encomendar ColeÃ§Ã£o</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-6 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-4xl md:rounded-3xl shadow-2xl h-full md:h-[90vh] flex flex-col overflow-hidden relative animate-scaleUp">
            ...
            <header className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <h2 className="font-serif text-2xl font-bold text-slate-800">Monte o seu Presente âœ¨</h2>
              <button onClick={closeModal} className="w-8 h-8 bg-slate-200 rounded-full flex items-center justify-center hover:bg-rose-600 hover:text-white transition-colors">âœ•</button>
            </header>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-10 bg-white">
              ...
              <div className="flex items-center justify-between mb-10 relative">
                ...
                {[ { n: 1, t: 'Embalagem' }, { n: 2, t: 'Produtos' }, { n: 3, t: 'CartÃ£o & Resumo' } ].map(s => (
              ...
              {customStep === 1 && (
                <div className="animate-fadeIn">
                  <h3 className="font-serif text-2xl text-slate-800 mb-6">1. Como vocÃª quer entregar?</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    {basketTypes.map(b => (
                      <div key={b.id} onClick={() => setSelectedBasket(b)} className={`cursor-pointer border-2 rounded-2xl p-4 transition-all ${selectedBasket?.id === b.id ? 'border-rose-500 bg-rose-50' : 'border-slate-100 hover:border-rose-300'}`}>
                        ...
                        <div className="w-full h-32 bg-slate-100 rounded-lg mb-4 flex items-center justify-center text-3xl">{b.name.includes('Caixa') ? 'ðŸŽ' : b.name.includes('Vime') ? 'ðŸ§º' : 'ðŸªš'}</div>
                  ...
                  <div className="mt-10 flex justify-end">
                    <button onClick={() => setCustomStep(2)} disabled={!selectedBasket} className="bg-slate-900 text-white px-8 py-3 rounded-lg font-bold disabled:opacity-50 hover:bg-rose-600 transition-colors">AvanÃ§ar â†’</button>
                  </div>
                </div>
              )}
              ...
              {customStep === 2 && (
                <div className="animate-fadeIn">
                          ...
                          {items.map(item => {
                            ...
                            return (
                              <div key={item.id} onClick={() => toggleItem(item)} className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition-all ${isSelected ? 'border-rose-500 bg-rose-50' : 'border-slate-200 hover:border-rose-300'}`}>
                                ...
                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${isSelected ? 'border-rose-600 bg-rose-600 text-white' : 'border-slate-300 text-transparent'}`}>âœ“</div>
                              </div>
                            );
                          })}
                        </div>
                  ...
                  <div className="mt-10 flex justify-between items-center border-t border-slate-100 pt-6">
                    <button onClick={() => setCustomStep(1)} className="text-slate-500 font-bold hover:text-slate-900">â† Voltar</button>
                    <div className="flex items-center gap-4">
                      ...
                      <button onClick={() => setCustomStep(3)} disabled={selectedItems.length === 0} className="bg-slate-900 text-white px-8 py-3 rounded-lg font-bold disabled:opacity-50 hover:bg-rose-600 transition-colors">AvanÃ§ar â†’</button>
                    </div>
                  </div>
                </div>
              )}

              {/* STEP 3: Resumo e CartÃ£o */}
              {customStep === 3 && (
                <div className="animate-fadeIn">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                      <label className="block font-bold text-sm text-slate-800 mb-2">Mensagem do CartÃ£o Afetivo (Opcional)</label>
                      <textarea 
                        value={customMessage} 
                        onChange={e => setCustomMessage(e.target.value)}
                        placeholder="Escreva uma mensagem especial que colocaremos em um lindo cartÃ£o Ã  mÃ£o..."
                  ...
                  <div className="mt-10 flex justify-between items-center border-t border-slate-100 pt-6">
                    <button onClick={() => setCustomStep(2)} className="text-slate-500 font-bold hover:text-slate-900">â† Editar Itens</button>
